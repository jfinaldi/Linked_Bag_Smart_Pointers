...
void displayBag(const shared_ptr<LinkedBag<string>>&);
...
int main() {
  ...
  cout << "--->>>>> Test 1 --->>>>>" << endl;
  shared_ptr<LinkedBag<string>> bag = make_shared <LinkedBag<string>>();
  ...
  // 10. Move values to a vector of smart pointers and output addresses
  cout << "\n--->>>>> Test 10 --->>>>>";
  bag->findLocations("4-FOUR");
  displayBag(bag);
  bag->findLocations("9-NINE");
  displayBag(bag);
...
void displayBag(const shared_ptr<LinkedBag<string>>& bag) {
  cout << "\n !Display bag: ";
