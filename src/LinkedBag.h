...
public:
  ...
  bool findLocations(const ItemType &);
private:
  ...
  bool removeDuplicateHelper(shared_ptr<Node<ItemType>>, ItemType &);
  vector<shared_ptr<Node<ItemType>>> findLocationsHelper(shared_ptr<Node<ItemType>>);
/*----------------------------------------------------------------------*/
...
private:
  std::shared_ptr<Node<ItemType>> headPtr{ std::make_shared<Node<ItemType>> (nullptr) }; // Pointer to first node
